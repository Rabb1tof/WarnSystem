...
char g_sSQL_CreateTablePlayers_SQLite[] = "CREATE TABLE IF NOT EXISTS `ws_player` ( \
    ...
    g_sSQL_CreateTablePlayers_MySQL[] = "CREATE TABLE IF NOT EXISTS `ws_player` ( \
        `account_id` int(12) NOT NULL default ''  COMMENT 'Steam AccountID', \
        `username` VARCHAR(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL default '', \
        `warns` INTEGER(10) unsigned NOT NULL DEFAULT '0', PRIMARY KEY (account_id)) \
        CHARSET=utf8 COLLATE utf8_general_ci COMMENT = 'Перечень всех игроков';",
    ...
    g_sSQL_CreateTableServers[] = "CREATE TABLE IF NOT EXISTS `ws_server` (`server_id` int(12) NOT NULL AUTO_INCREMENT, \
        `address` VARCHAR(32) NOT NULL default '', \
        `port` INTEGER(5) NOT NULL default '', \
        PRIMARY KEY (server_id) UNIQUE KEY `ws_servers_address_port` (`address`,`port`)) \
    ...
    g_sSQL_UploadData[] = "INSERT OR IGNORE INTO `ws_player` (`account_id`, `username`, `warns`) VALUES ('%i', '%s', '%i');",
    g_sSQL_UpdateData[] = "UPDATE OR IGNORE `ws_player` SET `username` = '%s', `warns` = '%i' WHERE `account_id` = '%i';",
...
public void SQL_TransactionSuccefully(Database hDatabase, any data, int iNumQueries, Handle[] hResults, any[] queryData)
{
    ...
    switch(data) {
        ...
        case 2:     szQuery = "Load Data";
        case 3:     szQuery = "Warn Player";
        case 4:     szQuery = "Unwarn Player";
    }
    PrintToServer("-----------------------------------------------------");
...
public void SQL_UnWarnPlayer(Database hDatabase, DBResultSet hDatabaseResults, const char[] sError, Handle hUnwarnData)
{
    ...
    if (hDatabaseResults.FetchRow())
    {
        ...
        g_hDatabase.Execute(hTxn, SQL_TransactionSuccefully, SQL_TransactionFailed, 4);
